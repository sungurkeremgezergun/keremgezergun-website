...
export const metadata: Metadata = {
  ...
  twitter: {
    ...
    title: "Knotvo — Destek",
    description:
      "Knotvo destek ve sık sorulan sorular: HAR dosyası, canlı ölçüm ve güvenli paylaşım.",
...
const pageSchema = productPageSchema({
  ...
  language: 'tr',
  name: 'Knotvo — Destek',
  description: 'Knotvo destek ve sık sorulan sorular. HAR dosyası nasıl alınır, canlı ölçüm nasıl çalışır, veriler nasıl sıfırlanır ve Knotvo ile ilgili yardım.',
  url: PAGE_URL,
});
...
export default function KnotvoDestekPage() {
  return (
    <main id="main-content" tabIndex={-1} className="knotvo-legal">
      ...
      <p>
        HAR (HTTP Archive), bir web sayfasının ağ isteklerinin kaydıdır.
        Chrome/Edge/Safari geliştirici araçlarında <em>Network</em> sekmesini
        açıp sayfayı yenileyin, sonra sağ tık → <em>Save all as HAR</em>. Bu
        dosyayı Knotvo&apos;da açıp analiz edin.
      </p>

      <p className="q">Verilerim Mac&apos;imden çıkıyor mu?</p>
      <p>
        HAR analizi %100 yereldir — hiçbir şey yüklenmez. Opsiyonel{" "}
        <em>canlı URL ölçümü</em> özelliği yalnızca girdiğiniz URL&apos;i Google
        PageSpeed Insights&apos;a gönderir.{" "}
        <Link href="/knotvo-gizlilik">Gizlilik Politikası</Link>&apos;na bakın.
      </p>

      <p className="q">
        Canlı ölçüm &quot;kota doldu&quot; diyor veya anahtar istiyor.
      </p>
      <p>
        Canlı ölçüm, Google&apos;ın ücretsiz PageSpeed Insights API&apos;sini
        kullanır. Google Cloud Console&apos;dan ücretsiz bir API anahtarı
        oluşturun (kredi kartı gerekmez, günde 25.000 sorgu) ve{" "}
        <em>Ayarlar → PageSpeed Insights API Anahtarı</em>&apos;na ekleyin.
      </p>

      <p className="q">
        localhost / staging / şifreli bir sayfayı test edebilir miyim?
      </p>
      <p>
        Canlı ölçüm yalnızca herkese açık URL&apos;lerde çalışır (Google sayfaya
        erişebilmeli). Özel veya iç sayfalar için tarayıcınızda HAR kaydı alıp
        Knotvo&apos;da açın.
      </p>

      <p className="q">Sır içeren bir HAR&apos;ı güvenle nasıl paylaşırım?</p>
      <p>
        <em>Sanitize → Paylaşıma Güvenli Kopya</em> kullanın. Knotvo çerezleri,
        Authorization başlıklarını, token&apos;ları ve gövde sırlarını kaldırıp
        yerel olarak temiz bir kopya üretir; orijinal dosyanız hiç değişmez.
      </p>

      <p className="q">Her şeyi nasıl sıfırlarım?</p>
      <p>
        <em>Ayarlar → Tüm veriyi sıfırla</em> tüm markaları, siteleri,
        taramaları, API anahtarınızı ve tercihlerinizi kalıcı olarak siler.
      </p>
